...
use market_simulator::{stock::{registry, Symbol}, AgentType, Market, Marketable, OrderBook};
            ...
            .show(ctx, |ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    ComboBox::from_label("Symbol")
                        ...
                        .show_ui(ui, |ui| {
                            for symbol in &self.available_symbols {
                                ui.selectable_value(&mut self.selected_symbol, symbol.clone(), symbol.clone());
                            }
                        });
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        ...
                        let start_stop_button = if self.is_market_running {
                            egui::Button::new(RichText::new("⏸ Pause").color(Color32::WHITE)).fill(Color32::from_rgb(220, 53, 69))
                        } else {
                            egui::Button::new(RichText::new("▶ Start").color(Color32::WHITE)).fill(Color32::from_rgb(40, 167, 69))
                        };
                        if ui.add(start_stop_button.rounding(Rounding::same(8.0))).clicked() {
                            self.is_market_running = !self.is_market_running;
                        }
                        let reset_button = egui::Button::new(RichText::new("🔄 Reset").color(Color32::WHITE)).fill(Color32::from_rgb(108, 117, 125)).rounding(Rounding::same(8.0));
                        if ui.add(reset_button).clicked() { self.reset_simulation(); }
                    });
                });
                ui.add_space(8.0);
            });
            ...
            ui.vertical(|ui| {
                ...
                egui::Grid::new("bids_grid").spacing([20.0, 4.0]).show(ui, |ui| {
                    ...
                    for (price, level) in order_book.bids.iter().rev().take(10) {
                        ...
                        ui.label(RichText::new(format_number(level.total_volume as i32)).monospace());
                        ui.end_row();
                    }
                });
            });
            ...
            Plot::new("price_history_plot").legend(Legend::default()).show_axes([true, true]).show_grid([true, true]).show(ui, |plot_ui| {
                if let Some(history) = self.price_histories.get(&self.selected_symbol) {
                    ...
                    if let Some(&last_price) = history.last() {
                        ...
                        plot_ui.points(Points::new(vec![[x, last_price]]).radius(radius as f32).color(Color32::from_rgba_unmultiplied(255, 215, 0, alpha)));
                        let ring_radius = 6.0 + pulse * 2.0;
                        plot_ui.points(Points::new(vec![[x, last_price]]).radius(ring_radius as f32).color(Color32::from_rgba_unmultiplied(255, 215, 0, (64.0 * (1.0 - pulse)) as u8)).filled(false));
                    }
                }
            });
        });
        ...
        if let (Some(bid), Some(ask)) = (best_bid, best_ask) {
            if ask > bid {
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.separator();
                    // NEW: ATH and ATL for the selected symbol
...
fn main() -> Result<(), eframe::Error> {
    ...
    let participants = vec![AgentType::MarketMaker, AgentType::DumbLimit, AgentType::DumbMarket, AgentType::WhaleAgent];
