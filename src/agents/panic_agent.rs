...
impl PanicAgent {
    pub fn new(
        ...
        println!("[Panic Agent {}] Monitoring {} stocks for feedback loops", 
                 id, monitored_stocks.len());
    ...
    fn update_baseline_prices(&self, view: &MarketState) {
        ...
        for &stock_id in &self.monitored_stocks {
            ...
            if current_baseline == 0.0 {
                ...
                println!("[Panic Agent {}] Set baseline for stock {}: ${:.2}", 
    ...
    fn check_for_panic_triggers(&self, view: &MarketState) -> Vec<(u64, f64)> {
        ...
        for &stock_id in &self.monitored_stocks {
            ...
            let current_price = view.last_traded_price.get(&stock_id).copied()
                .or_else(|| view.stocks.get_stock_by_id(stock_id).map(|s| s.initial_price))
                .unwrap_or(0.0);
            ...
            if price_change <= -PANIC_THRESHOLD_DROP {
                ...
                println!("[Panic Agent {}] PANIC TRIGGER: Stock {} dropped {:.1}% from baseline", 
                         self.id, stock_id, price_change * 100.0);
            }
        }
    ...
    fn execute_panic_selling(&self, stock_id: u64, price_drop: f64) {
        ...
        println!("[Panic Agent {}] 🚨 PANIC SELLING: {} shares of stock {} ({}% drop triggers {}x amplification)", 
                 self.id, panic_volume, stock_id, price_drop.abs() * 100.0, panic_intensity);
    }
    ...
    fn check_panic_recovery(&self, view: &MarketState) {
        ...
        for &stock_id in &self.monitored_stocks {
            if !panic_mode.get(&stock_id).copied().unwrap_or(false) { continue; }
            ...
            if price_change > -0.02 {
                panic_mode.insert(stock_id, false);
                println!("[Panic Agent {}] 📈 PANIC RECOVERY: Stock {} back to normal", 
                         self.id, stock_id);
            }
        }
    }
