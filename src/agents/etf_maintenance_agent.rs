...
impl ETFMaintenanceAgent {
    pub fn new(
        ...
        println!("[ETF Maintenance Agent {}] Managing ETF {} ({}) with {} constituents", 
                 id, etf_stock.ticker, etf_stock_id, constituent_stock_ids.len());
        println!("  - Maintenance threshold: {}bps", ETF_MAINTENANCE_THRESHOLD_BPS);
    ...
    fn calculate_maintenance_opportunity(&self, nav: f64, etf_price: f64) -> Option<(Side, f64, String)> {
    ...
    fn execute_maintenance_arbitrage(&self, etf_side: Side, magnitude_bps: f64, action_type: String) {
        // Check inventory limits before trading
        if !self.check_inventory_limits() {
            println!("[ETF Maintenance Agent {}] Skipping arbitrage - inventory limits exceeded", self.id);
            return;
        }
        
        // Calculate volume based on magnitude of opportunity
        let base_volume = rand::thread_rng().gen_range(ETF_MAINTENANCE_VOLUME_MIN..=ETF_MAINTENANCE_VOLUME_MAX);
        ...
        *self.last_arbitrage_time.write().unwrap() = std::time::Instant::now();
        ...
        println!("[ETF Maintenance Agent {}] {} Arbitrage: {:?} ETF vs {:?} constituents | {}bps opportunity | Volume: {}", 
    ...
    fn rebalance_inventory(&self) {
        ...
        for (&stock_id, &current_pos) in inventory.iter() {
            ...
            if imbalance.abs() > 1000 { // Only rebalance significant imbalances
                ...
                println!("[ETF Maintenance Agent {}] Rebalancing: {:?} {} shares of stock {}", 
...
impl Agent for ETFMaintenanceAgent {
    fn run(&mut self) {
        ...
        loop {
            ...
            thread::sleep(std::time::Duration::from_millis(1)); // Check every 10ms (very fast)
        }
    }
    ...
    fn decide_actions(&mut self) {
        ...
        *self.last_etf_price.write().unwrap() = etf_price;
        ...
        if let Some((etf_side, magnitude_bps, action_type)) = self.calculate_maintenance_opportunity(nav, etf_price) {
    ...
    fn get_pending_orders(&self) -> Vec<Order> { Vec::new() }
    ...
    fn evaluate_port(&mut self, view: &MarketState) -> f64 {
        ...
        println!("[ETF Maintenance Agent {}] Portfolio: ${:.2}M | Cash: ${:.2}M | Profit: ${:.2}K", 
                 self.id, portfolio_value / 1_000_000.0, cash / 1_000_000.0, total_profit / 1_000.0);
