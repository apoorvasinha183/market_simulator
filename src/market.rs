...
mod tests {
    ...
    impl Agent for TestAgent {
        ...
        fn buy_stock(&mut self, _vol: u64) -> Vec<OrderRequest> { vec![] }
        fn sell_stock(&mut self, _vol: u64) -> Vec<OrderRequest> { vec![] }
        fn cancel_open_order(&mut self, _id: u64) -> Vec<OrderRequest> { vec![] }
        fn get_id(&self) -> usize { self.id }
        fn clone_agent(&self) -> Box<dyn Agent> { Box::new(TestAgent::default()) }
    }
    ...
    fn test_end_to_end_trade_and_inventory_update() {
        ...
        let mut seller = Box::new(TestAgent { id: 0, inventory: 100, ..Default::default() });
        seller.requests.push(OrderRequest::LimitOrder { agent_id: 0, side: Side::Sell, price: 15000, volume: 50 });
        
        let mut buyer = Box::new(TestAgent { id: 1, inventory: 0, ..Default::default() });
        buyer.requests.push(OrderRequest::MarketOrder { agent_id: 1, side: Side::Buy, volume: 30 });
        ...
        assert_eq!(seller_final.get_inventory(), 70, "Seller's inventory should decrease by 30.");
        assert_eq!(buyer_final.get_inventory(), 30, "Buyer's inventory should increase by 30.");
        assert_eq!(market.last_traded_price, 150.00, "Last traded price should be updated.");
    }
    ...
    fn test_order_acknowledgement_flow() {
        ...
        let mut agent = Box::new(TestAgent { id: 0, ..Default::default() });
        agent.requests.push(OrderRequest::LimitOrder { agent_id: 0, side: Side::Buy, price: 14900, volume: 10 });
        ...
        assert_eq!(pending_orders.len(), 1, "Agent should have one acknowledged order.");
        let ack_order = &pending_orders[0];
        assert_eq!(ack_order.id, 1, "The acknowledged order should have a valid, non-zero ID.");
    }
}
